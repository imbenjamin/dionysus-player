...
**Verified:** the full suite (193 tests) has been run for real via
...
| `JellyfinAPIClient` | `JellyfinAPIClientTests.swift` | Request construction (paths, query params, auth headers), response decoding, HTTP/decoding error handling, and the `collectionsContaining` fan-out logic — all against a fake server (see below), not a real one. |
