...
**Verified:** the full suite (238 tests) has been run for real via
...
| `SearchResult` | `SearchResultTests.swift` | `SearchHint`→display mapping: subtitle text per item type (year for Movie/Series, parent series name for Episode, none for BoxSet), and image URL resolution (Thumb preferred over Primary when both are present, falling back to Primary alone, `nil` when neither tag exists). |
