...
| `LoginViewModel` | `LoginViewModelTests.swift` | `canSubmit` gating, delegation to `AppState.signIn`, the user-facing error message on failure. Quick Connect availability following `/QuickConnect/Enabled`, and staying hidden when that check fails rather than offering a button that could only fail. |
...
`customHTTPPort`, `quickConnectDisabled`, `quickConnectExpiring` and
`quickConnectPending`.
...
can tell the replacement code from the expired one.
