...
- **Most user journeys.** There *is* a UI suite now (see "UI tests" below),
  ...
  see `PlaylistItemList.onRemove`'s doc comment), and the
...
(`JellyfinAPIClient.playlistUserPermissions`) answers 404 and any playlist
item removal is refused with 403 — gating `PlaylistItemList`'s remove
affordances (`AssetDetailViewModel.canEditPlaylist`). Unlike `canDelete`,
there's no server field this app can read to derive that permission
locally (Jellyfin never exposes a playlist's `OwnerUserId` to a client), so
the stub mirrors the real permission-lookup endpoint rather than just
flipping a flag on the fixture DTOs.
